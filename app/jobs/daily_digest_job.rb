class DailyDigestJob < ApplicationJob
  ...
  def perform
    User.where(role: "manager").find_each do |manager|
      ...
      open_submissions = FeedbackSubmission.for_csrs(names).open.order(created_at: :asc)
      next if submissions.empty? && open_submissions.empty?

      begin
        ManagerDigestMailer.daily(manager, submissions, open_submissions).deliver_now
